...
Color color_error = {255, 100, 100, 255};
Color color_exito = {100, 255, 100, 255};
...
int formulario(Texture2D fondo, Font fuente1, Font fuente2, Font fuente3);
...
int formulario(Texture2D fondo, Font fuente1, Font fuente2, Font fuente3)
{
    ...
    // BOTONES
    Rectangle boton_aceptar = {550, 720, 200, 60};
    Rectangle boton_cancelar = {850, 720, 200, 60};

    char titulo[65];
    char artista[45];
    char duracion[8];
    char ruta_imagen[255];
    char ruta_audio[255];
    char ruta_video[255];
    ...
    while (!WindowShouldClose() && !formulario_completado)
    {
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            ...
            if (CheckCollisionPointRec(posicion_mouse, (Rectangle){200, 275, 550, 50}))
                enfocado = 0;
            else if (CheckCollisionPointRec(posicion_mouse, (Rectangle){200, 375, 550, 50}))
                enfocado = 1;
            else if (CheckCollisionPointRec(posicion_mouse, (Rectangle){200, 475, 200, 50}))
                enfocado = 2; // Campo de duración
            else if (CheckCollisionPointRec(posicion_mouse, (Rectangle){850, 420, 550, 50}))
                enfocado = 3;
            else if (CheckCollisionPointRec(posicion_mouse, (Rectangle){850, 555, 550, 50}))
                enfocado = 4;
        ...
        DrawTextEx(fuente2, "DURACION [MM:SS]:", (Vector2){ANCHO_PANTALLA*0.05, 470}, TAMANIO_FUENTE_TER, 0, color_verde);
        DrawRectangleRounded((Rectangle){ANCHO_PANTALLA*0.05, 510, 400, 60}, redondez, segmentos, enfocado == 2 ? color_campo_activo : color_campo_inactivo);
        DrawTextEx(fuente3, duracion, (Vector2){ANCHO_PANTALLA*0.06, 520}, TAMANIO_FUENTE_CUA, 0, enfocado == 2 ? color_texto_activo : color_texto_inactivo);
        ...
        if (strlen(duracion) > 0 && !duracion_valida)
        {
            DrawTextEx(fuente3, "FORMATO INVALIDO. \nUSE MM:SS", (Vector2){520, 520}, TAMANIO_FUENTE_QUI, 0, color_error);
        }

        // CASILLAS DE MARCA (RADIO BUTTONS)
        DrawTextEx(fuente2, "SELECCIONE UNA OPCION:", (Vector2){ANCHO_PANTALLA*0.05, 580}, TAMANIO_FUENTE_TER, 0, color_verde);
        ...
        DrawTextEx(fuente2, "INSERTAR AL INICIO", (Vector2){ANCHO_PANTALLA*0.07, 620}, TAMANIO_FUENTE_CUA, 0, color_verde);
        ...
        DrawTextEx(fuente2, "INSERTAR AL FINAL", (Vector2){ANCHO_PANTALLA*0.07, 650}, TAMANIO_FUENTE_CUA, 0, color_verde);
        ...
        DrawRectangleRounded((Rectangle){ANCHO_PANTALLA*0.525, 400, ANCHO_PANTALLA*0.425, 60}, redondez, segmentos, enfocado == 3 ? color_texto_activo : color_texto_inactivo);
        DrawTextEx(fuente3, ruta_imagen, (Vector2){ANCHO_PANTALLA*0.535, 410}, TAMANIO_FUENTE_CUA, 0, enfocado == 3 ? color_texto_activo : color_texto_inactivo);
        ...
        // CAMPO PARA AUDIO
        DrawTextEx(fuente2, "AUDIO:", (Vector2){ANCHO_PANTALLA*0.525, 500}, TAMANIO_FUENTE_TER, 0, color_verde);
        DrawTextEx(fuente2, "(RUTA DEL ARCHIVO)", (Vector2){ANCHO_PANTALLA*0.565, 518}, TAMANIO_FUENTE_QUI, 0, color_verde);
        DrawRectangleRounded((Rectangle){ANCHO_PANTALLA*0.525, 540, ANCHO_PANTALLA*0.425, 60}, redondez, segmentos, enfocado == 4 ? color_campo_activo : color_campo_inactivo);
        DrawTextEx(fuente3, ruta_audio, (Vector2){ANCHO_PANTALLA*0.535, 550}, TAMANIO_FUENTE_CUA, 0, enfocado == 4 ? color_texto_activo : color_texto_inactivo);

        // Botón para cargar audio
        DrawRectangleRounded((Rectangle){ANCHO_PANTALLA*0.525, 610, 110, 25}, redondez, segmentos, CheckCollisionPointRec(posicion_mouse, (Rectangle){850, 615, 110, 25}) ? color_boton_pulsado : color_boton);
        DrawTextEx(fuente2, "CARGAR AUDIO", (Vector2){ANCHO_PANTALLA*0.535, 615}, TAMANIO_FUENTE_QUI, 0, CheckCollisionPointRec(posicion_mouse, (Rectangle){850, 615, 110, 25}) ? color_texto_activo : color_texto_inactivo);
